...
class AsyncLLMClient:
    ...
    async def call_llm(self, session: aiohttp.ClientSession, url: str, headers: Dict[str, str], 
                       request_data: Dict[str, Any], timeout: int = 300) -> Dict[str, Any]:
        """
        异步调用LLM服务
            ...
            try:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        raise RuntimeError(f"LLM服务返回错误状态码: {response.status}, 响应: {error_text}")
    ...
    async def batch_call_llm(self, requests: List[Dict[str, Any]], timeout: int = 300) -> List[Dict[str, Any]]:
        """
