import json
import re
import asyncio
from app.core.vectoring import VectorClient
from pymilvus import DataType


class StandardDataProcessor:
    """标准数据处理器，用于处理标准数据文件并存储到Milvus数据库"""

    def __init__(self):
        """初始化向量客户端"""
        self.vector_client = VectorClient()

    def setup_collection(self):
        """设置标准数据集合"""
        # 检查是否已存在名为"standard_telecom"的集合，如果存在则删除
        if self.vector_client.milvus_client.has_collection("standard_telecom"):
            self.vector_client.milvus_client.drop_collection("standard_telecom")
            print("已删除现有的 standard_telecom 集合")

        # 创建新的集合
        schema = self.vector_client.milvus_client.create_schema(
            auto_id=False,
            description="Standard data elements with embeddings"
        )

        # 添加字段
        schema.add_field("id", datatype=DataType.INT64, is_primary=True, auto_id=True)
        schema.add_field("vector", datatype=DataType.FLOAT_VECTOR, dim=self.vector_client.embedding_dim)
        schema.add_field("true_element", datatype=DataType.VARCHAR, max_length=65535)
        schema.add_field("classification", datatype=DataType.VARCHAR, max_length=65535)
        schema.add_field("grading", datatype=DataType.VARCHAR, max_length=65535)

        # 创建集合
        self.vector_client.milvus_client.create_collection(
            collection_name="standard_telecom",
            schema=schema,
            consistency_level="Strong"
        )

        # 创建向量索引
        index_params = self.vector_client.milvus_client.prepare_index_params()
        index_params.add_index(
            field_name="vector",
            index_type="FLAT",
            metric_type="IP"
        )
        self.vector_client.milvus_client.create_index(
            collection_name="standard_telecom",
            index_params=index_params
        )

    async def process_standard_data(self):
        """
        处理标准数据文件，提取所需字段并存储到Milvus数据库
        """
        # 设置集合
        self.setup_collection()

        # 读取数据文件
        data_file_path = "data/standards/standard.jsonl"
        data_to_insert = []

        with open(data_file_path, 'r', encoding='utf-8') as file:
            for line_num, line in enumerate(file, 1):
                # 跳过空行
                if not line.strip():
                    print(f"第 {line_num} 行为空行，跳过")
                    continue
                    
                try:
                    record = json.loads(line.strip())
                    header = record.get("header", {})
                    data = record.get("data", {})
                    
                    # 检查是否为空记录
                    if not header and not data:
                        print(f"第 {line_num} 行为空记录，跳过")
                        continue

                    # 获取各字段索引位置
                    level1_index = None
                    level2_index = None
                    level3_index = None
                    level4_index = None
                    level5_index = None
                    level6_index = None
                    level7_index = None
                    level8_index = None
                    level9_index = None
                    feature_index = None
                    grade_index = None
                    real_data_index = None

                    for idx, name in header.items():
                        if name == "一级分类":
                            level1_index = idx
                        elif name == "二级分类":
                            level2_index = idx
                        elif name == "三级分类":
                            level3_index = idx
                        elif name == "四级分类":
                            level4_index = idx
                        elif name == "五级分类":
                            level5_index = idx
                        elif name == "六级分类":
                            level6_index = idx
                        elif name == "七级分类":
                            level7_index = idx
                        elif name == "八级分类":
                            level8_index = idx
                        elif name == "九级分类":
                            level9_index = idx
                        elif name == "对应特征":
                            feature_index = idx
                        elif name == "等级":
                            grade_index = idx
                        elif name == "真实数据":
                            real_data_index = idx

                    # 提取真实数据
                    true_elements = data.get(real_data_index, [])
                    if isinstance(true_elements, list):
                        true_elements_list = true_elements
                    else:
                        # 如果不是列表，按逗号分割
                        true_elements_list = str(true_elements).split(',') if true_elements else []
                        # 清理空白字符
                        true_elements_list = [elem.strip() for elem in true_elements_list if elem.strip()]

                    # 提取分类信息（优先级：九级分类 > 八级分类 > 七级分类 > 六级分类 > 五级分类 > 四级分类 > 三级分类 > 二级分类 > 一级分类）
                    classification = ""
                    if level9_index is not None and data.get(level9_index, "").strip():
                        classification = data.get(level9_index, "")
                    elif level8_index is not None and data.get(level8_index, "").strip():
                        classification = data.get(level8_index, "")
                    elif level7_index is not None and data.get(level7_index, "").strip():
                        classification = data.get(level7_index, "")
                    elif level6_index is not None and data.get(level6_index, "").strip():
                        classification = data.get(level6_index, "")
                    elif level5_index is not None and data.get(level5_index, "").strip():
                        classification = data.get(level5_index, "")
                    elif level4_index is not None and data.get(level4_index, "").strip():
                        classification = data.get(level4_index, "")
                    elif level3_index is not None and data.get(level3_index, "").strip():
                        classification = data.get(level3_index, "")
                    elif level2_index is not None and data.get(level2_index, "").strip():
                        classification = data.get(level2_index, "")
                    elif level1_index is not None and data.get(level1_index, "").strip():
                        classification = data.get(level1_index, "")

                    # 提取等级信息并选择最高等级
                    grade_raw = data.get(grade_index, "")
                    grading = self.extract_highest_grade(grade_raw)

                    # 提取特征文本
                    feature_text = data.get(feature_index, "")

                    # 为每个真实元素创建一条记录
                    for element in true_elements_list:
                        # 构造特征+真实数据的字符串用于向量化
                        # vector_text = f"{feature_text},{element}" if feature_text else element
                        vector_text = element
                        # 准备数据插入
                        data_to_insert.append({
                            "true_element": element,
                            "classification": classification,
                            "grading": grading,
                            "text_for_vector": vector_text  # 临时字段，用于向量化
                        })

                except json.JSONDecodeError as e:
                    print(f"第 {line_num} 行 JSON 解析错误: {e}")
                except Exception as e:
                    print(f"处理第 {line_num} 行时发生错误: {e}")

        # 向量化文本
        texts_for_vector = [item.pop("text_for_vector") for item in data_to_insert]  # 移除临时字段
        embeddings = await self.vector_client.get_embeddings(texts_for_vector)

        # 将向量添加到数据中
        for i, embedding in enumerate(embeddings):
            data_to_insert[i]["vector"] = embedding

        # 插入数据到数据库
        if data_to_insert:
            result = self.vector_client.milvus_client.insert(
                collection_name="standard_telecom",
                data=data_to_insert
            )
            print(f"成功插入 {len(result['ids'])} 条记录到 standard_telecom 集合")
        else:
            print("没有数据需要插入")

        return len(data_to_insert)

    @staticmethod
    def extract_highest_grade(grade_text):
        """
        从等级文本中提取最高等级
        如"第4级/第3级/第2级"中提取"第4级"
        """
        if not grade_text:
            return ""

        # 查找所有类似"第N级"的模式
        grades = re.findall(r'第(\d+)级', grade_text)
        if grades:
            # 取最大的数字作为最高等级
            highest_grade = max(int(g) for g in grades)
            return f"第{highest_grade}级"
        else:
            return grade_text